...
def load_users():
    if not os.path.exists(USERS_FILE):
        default = {"admin": {"password": "OPTUFAN", "role": "OWNARTUFAN"}}
        save_users(default)
        return default
...
if __name__ == '__main__':
    # Render પોતે એક પોર્ટ આપશે, જો ન મળે તો ડિફોલ્ટ 5000 લેશે
    port = int(os.environ.get('PORT', 5000))
    
    print("\n" + "=" * 50)
    print("🚀 JUBAYER HOSTING - DEPLOYING ON RENDER")
    print("=" * 50)
    # નોંધ: Render પર તમારી લિંક .onrender.com વાળી હશે
    print(f"👤 Admin: admin | Pass: admin123")
    print("=" * 50 + "\n")

    # Render પર debug=False રાખવું સલાહભર્યું છે
